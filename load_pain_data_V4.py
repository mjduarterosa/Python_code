...
data_UK=import_mat_data_V4.import_mat_data( dname_UK)
data_JP=import_mat_data_V4.import_mat_data_h5( dname_JP)
data_US=import_mat_data_V4.import_mat_data( dname_US)

P_UK=import_mat_data_V4.glob.glob(dname_UK)
P_UK=[i for i in range(len(P_UK)) if (P_UK[i][99:-6:]=='P') ]

P_JP=import_mat_data_V4.glob.glob(dname_JP)
P_JP=[i for i in range(len(P_JP)) if (P_JP[i][108:-4:]=='P') ]

P_US=import_mat_data_V4.glob.glob(dname_US)
